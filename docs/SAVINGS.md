...
- **Short `squire run` passthroughs are not logged.** Output of 60 lines or fewer passes
  through unchanged and writes no ledger row, so call counts cover model-backed calls only.
