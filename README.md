...
2. **99.7%**: share of the characters sent to squire that were kept out of context, across 1,018 calls (MEASURED).
3. **~72.2M**: tokens kept out of context, from characters ÷ 4 (ESTIMATED).
...
**2026-09-15 to 2026-09-15** &mdash; 16 model-backed calls logged, 100.0% backend-ok (0.0% `UNKNOWN`), plus 43 short `run` passthrough runs (60 lines of output or fewer, shown raw with no model call and no savings counted).

| Command | Calls | Ok rate | `UNKNOWN` rate | Chars in | Chars out | Chars saved |
|---|---|---|---|---|---|---|
| `diff` | 2 | 100.0% | 0.0% | 21481 | 2070 | 19411 |
| `grep` | 6 | 100.0% | 0.0% | 10909586 | 13995 | 10895591 |
| `run` | 7 | 100.0% | 0.0% | 80471 | 4221 | 76250 |
| `sum` | 1 | 100.0% | 0.0% | 7172 | 1213 | 5959 |

**REAL character counts** (not estimated): 11018710 chars in, 21499 chars out, **10997211 chars kept out of context**.

ESTIMATED tokens saved: ~2749303 (chars/4 heuristic, not a measured token count -- see "Measuring savings" above for the fuller method).
