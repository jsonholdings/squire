...
2. **99.7%**: share of the characters sent to squire that were kept out of context, across 1,020 calls (MEASURED).
3. **~73.0M**: tokens kept out of context, from characters ÷ 4 (ESTIMATED).
...
**2026-09-15 to 2026-09-16** &mdash; 18 model-backed calls logged, 100.0% backend-ok (0.0% `UNKNOWN`), plus 57 short `run` passthrough runs (60 lines of output or fewer, shown raw with no model call and no savings counted).

| Command | Calls | Ok rate | `UNKNOWN` rate | Chars in | Chars out | Chars saved |
|---|---|---|---|---|---|---|
| `diff` | 2 | 100.0% | 0.0% | 21481 | 2070 | 19411 |
| `grep` | 7 | 100.0% | 0.0% | 13978959 | 16584 | 13962375 |
| `run` | 8 | 100.0% | 0.0% | 85704 | 4858 | 80846 |
| `sum` | 1 | 100.0% | 0.0% | 7172 | 1213 | 5959 |

**REAL character counts** (not estimated): 14093316 chars in, 24725 chars out, **14068591 chars kept out of context**.

ESTIMATED tokens saved: ~3517148 (chars/4 heuristic, not a measured token count -- see "Measuring savings" above for the fuller method).
