...
**Status: v0.2.21.** Every command in `squire --help` is implemented: `run`, `sum`,
...
<!-- SQUIRE-HEADLINE:BEGIN (generated by scripts/squire_report.py --write, do not hand-edit) -->
**Headline** (201 real sessions, aggregated — see SAVINGS.md for the full table):

1. **36,500:1**: cache-read tokens to uncached input tokens (MEASURED).
2. **99.9%**: share of the characters sent to squire that were kept out of context, across 1,162 calls (MEASURED).
3. **~200.9M**: tokens kept out of context, from characters ÷ 4 (ESTIMATED).
...
**2026-09-15 to 2026-09-18** &mdash; 160 model-backed calls logged, 98.8% backend-ok (1.2% `UNKNOWN`), plus 248 short `run` passthrough runs (60 lines of output or fewer, shown raw with no model call and no savings counted).
...
| `grep` | 33 | 97.0% | 3.0% | 523155416 | 64586 | 523090830 |
| `run` | 35 | 100.0% | 0.0% | 1723765 | 24033 | 1699732 |
| `sum` | 38 | 97.4% | 2.6% | 308515 | 37558 | 270957 |

**REAL character counts** (not estimated): 525972731 chars in, 175321 chars out, **525797410 chars kept out of context**.

ESTIMATED tokens saved: ~131449352 (chars/4 heuristic, not a measured token count -- see "Measuring savings" above for the fuller method).
