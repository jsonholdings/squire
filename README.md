...
**Headline** (198 real sessions, aggregated — see SAVINGS.md for the full table):

1. **36,000:1**: cache-read tokens to uncached input tokens (MEASURED).
2. **99.7%**: share of the characters sent to squire that were kept out of context, across 1,027 calls (MEASURED).
3. **~76.2M**: tokens kept out of context, from characters ÷ 4 (ESTIMATED).
...
**2026-09-15 to 2026-09-16** &mdash; 25 model-backed calls logged, 100.0% backend-ok (0.0% `UNKNOWN`), plus 99 short `run` passthrough runs (60 lines of output or fewer, shown raw with no model call and no savings counted).

| Command | Calls | Ok rate | `UNKNOWN` rate | Chars in | Chars out | Chars saved |
|---|---|---|---|---|---|---|
| `ask` | 2 | 100.0% | 0.0% | 37588 | 2141 | 35447 |
| `diff` | 2 | 100.0% | 0.0% | 21481 | 2070 | 19411 |
| `grep` | 11 | 100.0% | 0.0% | 26772761 | 23879 | 26748882 |
| `run` | 9 | 100.0% | 0.0% | 98914 | 6045 | 92869 |
| `sum` | 1 | 100.0% | 0.0% | 7172 | 1213 | 5959 |

**REAL character counts** (not estimated): 26937916 chars in, 35348 chars out, **26902568 chars kept out of context**.

ESTIMATED tokens saved: ~6725642 (chars/4 heuristic, not a measured token count -- see "Measuring savings" above for the fuller method).
