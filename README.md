...
**Headline** (196 real sessions, aggregated — see SAVINGS.md for the full table):

1. **36,500:1**: cache-read tokens to uncached input tokens (MEASURED).
2. **99.7%**: share of the characters sent to squire that were kept out of context, across 1,023 calls (MEASURED).
3. **~75.1M**: tokens kept out of context, from characters ÷ 4 (ESTIMATED).
...
**2026-09-15 to 2026-09-16** &mdash; 21 model-backed calls logged, 100.0% backend-ok (0.0% `UNKNOWN`), plus 96 short `run` passthrough runs (60 lines of output or fewer, shown raw with no model call and no savings counted).

| Command | Calls | Ok rate | `UNKNOWN` rate | Chars in | Chars out | Chars saved |
|---|---|---|---|---|---|---|
| `diff` | 2 | 100.0% | 0.0% | 21481 | 2070 | 19411 |
| `grep` | 10 | 100.0% | 0.0% | 22476331 | 21439 | 22454892 |
| `run` | 8 | 100.0% | 0.0% | 85704 | 4858 | 80846 |
| `sum` | 1 | 100.0% | 0.0% | 7172 | 1213 | 5959 |

**REAL character counts** (not estimated): 22590688 chars in, 29580 chars out, **22561108 chars kept out of context**.

ESTIMATED tokens saved: ~5640277 (chars/4 heuristic, not a measured token count -- see "Measuring savings" above for the fuller method).
